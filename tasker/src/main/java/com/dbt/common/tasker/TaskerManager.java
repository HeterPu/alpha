...
public class TaskerManager {
    ...
    private static TaskerManager sInstance = null;
    ...
    private TaskerManager(Context context) {
        if (context == null) {
     ...
     */
    public static synchronized TaskerManager getInstance(Context context) {
        if (sInstance == null) {
            sInstance = new TaskerManager(context);
        }
    ...
    public void start() {
        ...
        do {
            ...
            if (TaskerUtils.isInMainProcess(mContext)
        ...
        if (project != null) {
            ...
            TaskerLog.e(TaskerLog.GLOBAL_TAG, "No startup project for current process.");
        }
    }
    ...
    public void addProject(Task project, String processName) {
        if (TaskerUtils.isMatchProcess(mContext, processName)) {
    ...
    public void addProject(Task project, int mode) {
        ...
        if (TaskerUtils.isMatchMode(mContext, mode)) {
    ...
    public void addProjectsViaFile(File file,OnProjectExecuteListener projectListener,OnGetMonitorRecordCallback monitorListener) {
        if (!file.exists()) {
            ...
            throw new RuntimeException("Tasker config file " + file + " is not exist!");
        }
    ...
    public void executeAfterStartup(Task task) {
        executeAfterStartup(task, TaskerManager.ALL_PROCESS_MODE);
    }
     ...
     *
     * @param task 在启动结束时执行的{@code task}
     * @param mode 启动流程的执行模型。对应{@link TaskerManager#MAIN_PROCESS_MODE},
     *             {@link TaskerManager#ALL_PROCESS_MODE}, {@link TaskerManager#SECONDARY_PROCESS_MODE}
     */
    ...
    public void executeAfterTask(Task task, String taskName) {
        executeAfterTask(task, taskName, TaskerManager.ALL_PROCESS_MODE);
    }
    ...
    private void executeProjectBindRunnables() {
        TaskerUtils.sort(mExtraTaskList);

...
import static com.dbt.common.tasker.TaskerManager.ALL_PROCESS_MODE;
import static com.dbt.common.tasker.TaskerManager.MAIN_PROCESS_MODE;
import static com.dbt.common.tasker.TaskerManager.SECONDARY_PROCESS_MODE;
...
public class TaskerUtils {
    ...
    /**
     * 在{@link TaskerManager#addProject(Task, String)}中进行这个判断，只有进程名和当前进程相同，才有必要去持有该{@code Project}.
     *
    ...
    public static boolean isMatchProcess(Context context, String processName) {
        String currentProcessName = TaskerUtils.getCurrProcessName(context);
        return TextUtils.equals(processName, currentProcessName);
    }

    /**
     * 在{@link TaskerManager#addProject(Task, String)}中进行这个判断，只有当前进程命中指定的{@code mode}，才有必要去持有该{@code
     * Project}.
     *
     * @param context The context used to check process.
    ...
    public static boolean isMatchMode(Context context, int mode) {
        ...
        if (TaskerUtils.isInMainProcess(context) && mode == MAIN_PROCESS_MODE) {
            return true;
        }

        if (!TaskerUtils.isInMainProcess(context) && mode == SECONDARY_PROCESS_MODE) {
            return true;
        }

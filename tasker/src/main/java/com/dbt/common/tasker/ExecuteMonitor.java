...
class ExecuteMonitor {
    ...
    public synchronized void record(String taskName, long executeTime) {
        TaskerLog.d(TaskerLog.GLOBAL_TAG, "Task-->Startup task %s cost time: %s ms, in thread: %s", taskName, executeTime, Thread.currentThread().getName());
        if (executeTime >= TaskerConfig.getWarmingTime()) {
            toastToWarn("Task %s run too long, cost time: %s", taskName, executeTime);
        }
    ...
    public void recordProjectFinish(String projectName) {
        ...
        TaskerLog.d("==Tasker==", "ProjectName " + "【" + projectName + "】" + "tm start up cost time: %s ms", mProjectCostTime);
    }
    ...
    private void toastToWarn(final String msg, final Object... args) {
        if (TaskerConfig.shouldShowToastToAlarm()) {
            getHandler().post(new Runnable() {
                ...
                public void run() {
                    ...
                    Toast.makeText(TaskerConfig.getContext(), formattedMsg, Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
